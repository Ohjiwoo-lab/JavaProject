/*
 * [과제 14번] File 클래스 활용
 * 문제 13을 확장하여 파일과 디렉터리 이름을 변경하고 현재 디렉토리 밑에 새로운 디렉토릴를 생성하는 명령을 추가한다.
 */
...
public class Homework14 {
    ...
    public Homework14(String dirName) {  // 생성자
    ...
    private void showSubDirtories() {
        ...
        for(File f : subFiles) {
            System.out.print(((f.isFile())?"file":"dir"));  // 파일이면 file, 아니면 dir로 출력한다.
            System.out.printf("%-15s", "\t\t" + f.length() + "바이트");  // 파일 크기 출력
    ...
    private boolean contains(String filename) {
        for(File f : subFiles) {
            if(f.getName().equalsIgnoreCase(filename))  // 파일 이름이 있는지 검색
                return true;
        }
        return false;
    }
    ...
    public static void main(String[] args) {
        Homework14 fe = new Homework14("c:\\");  // 객체 생성
        fe.run();  // 실행
    }
}
